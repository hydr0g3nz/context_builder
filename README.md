...
| Impact analysis | ไม่มี | `impact <sym>` — blast radius + risk signals |
| Smart context | ไม่มี | `context <task>` — ranked symbol bundle จาก free text |
...
**ข้อกำหนดสำหรับ Phase 2–3 commands** (`callers`, `callees`, `trace`, `find-impls`, `find-iface`, `refs`, `impact`):
...
# Phase 3: AI-native queries
gocx impact "UserService.Save"       # blast radius + risk signals
gocx context "add JWT auth to login" # ranked context bundle จาก free text
```
...
คำสั่ง Phase 3 ออกแบบมาสำหรับ AI โดยเฉพาะ — return context bundle ที่ packed แล้ว ไม่ใช่ raw LSP data

`context` ทำงาน offline ได้ถ้า Phase 2 edges ถูก cache ไว้แล้ว (ไม่ต้อง spawn gopls ใหม่)

---

#### `gocx impact <symbol> [--depth N] [--output json|text]`

วิเคราะห์ blast radius ของการเปลี่ยน symbol — หา caller ทั้งหมด, แบ่งกลุ่ม, ประเมิน risk

```bash
gocx impact Save
gocx impact "UserService.Save" --depth 3
gocx impact HandleLogin --output text
```
...
```json
{
  ...
  "results": {
    "symbol": { "name": "UserService.Save", "kind": "method", ... },
    "direct_callers": [ ... ],
    "transitive_reach": 12,
    "risk_signals": [
      "called from HTTP handler (HandleCreate)",
      "high fan-in (12 callers)",
      "breaks 3 test(s) if changed"
    ],
    "breakable_tests": [ ... ]
  },
  "truncated": false,
  "token_estimate": 340,
  "next_actions": [
    "gocx trace HandleCreate UserService.Save",
    "gocx find-iface UserService"
  ]
}
```

Risk signals (heuristic):
- `"called from HTTP handler"` — caller name มี `servehttp`/`handler`/`handlefunc` หรือ file มี `handler`
- `"high fan-in (N callers)"` — transitive reach > 10
- `"breaks N test(s) if changed"` — caller file ลงท้ายด้วย `_test.go`

---

#### `gocx context <task> [--limit N] [--output json|text]`

แปลง free-text task description เป็น ranked symbol bundle — ให้ AI รู้ว่าต้องอ่านไฟล์ไหน

```bash
gocx context "add JWT authentication to login flow"
gocx context "fix user save validation" --limit 20
gocx context "refactor S3 upload" --output text
```

**JSON output:**
```json
{
  "query": "context add JWT authentication to login flow",
  "results": {
    "seeds": ["UserService", "Login"],
    "files": [
      {
        ...
        "symbols": [
          {
            ...
            "package": "user",
            "line": 42,
            "signature": "func (s *UserService) Login(ctx context.Context, ...) (*Token, error)",
            "score": 1.0
          }
        ]
      }
    ],
    "summary": "Task touches 2 package(s), 8 symbols seeding from UserService, Login; focus on internal/user/service.go"
  },
  "truncated": false,
  "token_estimate": 680,
  "next_actions": [
    "gocx callers UserService.Login",
    "gocx impact UserService.Login"
  ]
}
```

Algorithm:
1. **Seed** — extract CamelCase tokens จาก task text → `find_symbols` lookup
2. **Expand** — BFS depth-1 บน cached `CALLS` + `IMPLEMENTS` edges (ทั้งสองทิศทาง)
3. **Rank** — score = `0.6 × distance_score + 0.4 × centrality_score`
4. **Pack** — group by file path, sort by score
...
- `gocx context "<task>"` to get a ranked list of relevant files for a task
- `gocx impact <name>` to assess the blast radius before changing a symbol
...
| 3 | ✅ Complete | `impact <sym>`, `context <task>`, risk signals, ranked context bundles, `next_actions` |
...
│   ├── impact.rs        # gocx impact   (Phase 3)
│   ├── context.rs       # gocx context  (Phase 3)
...
├── impact/              # (Phase 3) blast-radius analysis
│   └── mod.rs           # ImpactReport + risk signal heuristics
├── context/             # (Phase 3) smart context builder
│   ├── mod.rs           # orchestrate seed→expand→rank→pack
│   ├── seed.rs          # text → seed symbols
│   ├── expand.rs        # BFS over cached edges
│   ├── rank.rs          # distance + centrality scoring
│   └── pack.rs          # group by file, build summary
├── index/
